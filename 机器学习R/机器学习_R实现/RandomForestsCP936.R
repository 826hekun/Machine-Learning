...
library(randomForest) #随机森林的工具包
# install.packages("pROC") # 画ROC曲线的工具包
library(pROC)
...
mode1 
plot(mode1)
mode3 <-randomForest(良.恶~ .,data=train_data,ntree=50000,mtry=7) # ntree 数的棵树，mtry 每棵树使用的特征个数
mode3
varImpPlot(mode3) #画出来变量的重要性
x=data.frame(mode3$importance)
library(dplyr) 
...
data2=read.csv("E:/讲课/机器学习_R语言实现/multiclasstest.csv")
data2=data2[,-1]
data2=na.omit(data2)
View(data2)
...
data=read.csv("E:/讲课/机器学习_R语言实现/lung.csv")
summary(data)
# 数据预处理，异常值\缺失值处理
...
mode5<- randomForest(time ~ ., data=train_data, ntree=5000, mtry=3,
                     importance=TRUE)
...
for (i in 1:205){
  ...
  prediction1[i]=sum(predict1[i,])
}
plot(c(1:205),train_data[,1],col='blue',pch=1,lwd=4,type='b')
lines(c(1:205),exp(prediction1),col='red',pch=1,lwd=4,type='b')
# 绘制KM曲线
